...
#Skill validation
# Add this above the function (near the top of the file, with the other constants)
SPOKEN_LANGUAGES = {
    ...
    'urdu', 'kannada', 'malayalam', 'italian',
}
...
def validate_skills_with_projects(
    ...
    for skill in skills:
        # ── NEW: spoken languages don't need project evidence ──
        if skill.strip().lower() in SPOKEN_LANGUAGES:
            validated_skills.append({
                'skill': skill,
                'projects': ['Language proficiency — no project evidence required'],
                'similarity': 1.0,
            })
            skill_project_mapping[skill] = ['Language proficiency']
            continue
        # ── end of new code ──
